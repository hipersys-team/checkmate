...
class CFCheckpoint:
    ...
    def __init__(self, **kwargs):
        ...
        for name, ref in kwargs.items():
            if hasattr(ref, "state_dict"):
                ...
                self.logger.info(
                    "Skipping object `{}` in CF Checkpointing. \
				No state_dict() method exposed".format(
                        name
                    )
                )
    ...
    def _restore(self, filepath="model.chk", gpu=0):
        ...
        checkpoint = torch.load(
            filepath, map_location=lambda storage, loc: storage.cuda(gpu)
        )
...
def update_stats(
    filepath, iter_chk=None, epoch_chk=None, overwrite=True, linkpath=None
):
