...
from checkfreq.cf_manager import CFMode
from checkfreq.disk_bw import get_storage_bandwidth
import json
import os
...
class CFIterator:
    def __init__(
        ...
        if not isinstance(dataloader, Iterable):
            raise ValueError(
                "Dataloader of type {} is not iterable".format(type(dataloader))
            )
        ...
        self._total_steps = (
            self._epoch * self._max_steps_per_epoch + self._steps_this_epoch
        )
        ...
        print(
            "Epoch={}, this ep steps={}, max_steps={}, total_steps={}, steps_to_run = {}".format(
                self._epoch,
                self._steps_this_epoch,
                self._max_steps_per_epoch,
                self._total_steps,
                self._steps_to_run,
            )
        )
    ...
    def load_params_from_cache(self):
        ...
        if found_all:
            ...
            print(
                "Loaded : iter_dur = {}s, freq={}, fn={}, th={}".format(
    ...
    def __next__(self):
        ...
        if (
            self._worker_id == 0
            and self._chk_mode == CFMode.AUTO
        ...
        # Checkpoint if required on main worker
            ...
            self._worker_id == 0
            and self._chk_freq > 0
            and self._steps_since_chk == self._chk_freq
        ):
            ...
            print(
                "MUST CHECKPOINT NOW AT ITER {}, steps {}".format(
                    self._steps_this_epoch, self._steps_since_chk
                )
            )
            if self._chk_mode == CFMode.MANUAL:
                ...
                # Iter-level chk
                self._chk_fn(
                    additional_snapshot=self.state_dict(), use_thread=self._use_thread
                )
            ...
            if (
                self._profile_done
                and not self._start_monitor
                and self._chk_mode == CFMode.AUTO
                and not self._stop_monitor
            ):
                self._iter_dur = []
        ...
        if self._worker_id == 0 and self._start_monitor and not self._stop_monitor:
            ...
            if (
                self._monitor_iter_count > 0
                and self._monitor_iter_count <= self._chk_freq
            ):
                self._iter_dur.append(time.time() - self._prev_iter_end)
            elif self._monitor_iter_count == self._chk_freq + 1:
                ...
                print(
                    "NEW OVERHEAD IS  ={:.2f}, FULL={:.2f}".format(
                        overhead, overhead_full
                    )
                )
                print(self._iter_dur)
                print(
                    "OLD ITER={:.2f}, NEW_ITER={:.2f}".format(
                        self._avg_iter_dur, current_iter_mean
                    )
                )
                print(
                    "OLD TOTAL={:.2f}, NEW_TOTAL={:.2f}, percent={:.2f}%".format(
                        orig_total, current_total, overhead_percent
                    )
                )
        ...
        if self._steps_to_run >= 0:
            if self._steps_this_run == self._steps_to_run:
                ...
                self._exit = True
                print(
                    "Epoch={}, this ep steps={}, total_steps={}, steps_this_run = {}".format(
                        self._epoch,
                        self._steps_this_epoch,
                        self._total_steps,
        ...
        try:
            ...
            print(
                "Tracking epoch end in CF DL. Steps this run = {}, this epoch={}, samples this epoch={}".format(
                    self._steps_this_run,
                    self._steps_this_epoch,
                    self._samples_this_epoch,
                )
            )
            self._epoch += 1
            self._steps_this_run = 0
    ...
    def _complete_profile(self, dev):
        ...
        self._avg_free_mem = (
            (torch.cuda.get_device_properties(dev).total_memory - mean(self._mem_util))
            / 1024
            / 1024
        )
        print(
            "Average iter dur = {:.3f}, free mem = {:.2f}MB".format(
                self._avg_iter_dur, self._avg_free_mem
            )
        )
        ...
        if self._chk_size < self._avg_free_mem:
            ...
            print(
                "t_fp={:.4f}, t_ft={:.4f}, t_gp={:.4f}, t_gt={:.4f}".format(
                    t_fp, t_ft, t_gp, t_gt
                )
            )
            t_s = t_f - t_g
        ...
        print(
            "t_c={:.4f}, t_g={:.4f}, t_s={:.4f}, t_w={:.4f}".format(t_c, t_g, t_s, t_w)
        )
        ...
        print(
            "Chosen freq = {}, percent_ov={:.2f}%".format(
                self._chk_freq, percent_overhead
            )
        )
    ...
    def profile_all(self):
        ...
        dur_snap_1, full_proc_gpu_time = self._cf_manager.save(
            profile=True, use_thread=False
        )
        dur_snap_2, full_th_gpu_time = self._cf_manager.save(
            profile=True, use_thread=True
        )
        print(
            "Snap th={:.2f}\nSnap proc={:.2f}\nFull Th={:.2f}\nFull proc={:.2f}\nFull GPU proc={:.2f}\nFull GPU th={:.2f}\nGPU snap 1= {:.2f}\nGPU snap 2={:.2f}".format(
                snap_thread_time,
                snap_proc_time,
                full_thread_time,
                full_proc_time,
