...
class CFManager:
    ...
    def __init__(
        ...
        self.active_snapshot = self.mp_context.Value("i", 0)
        self.lock = self.mp_context.Lock()
        self.in_progress_snapshot = self.mp_context.Value("i", 0)
    ...
    def save(
        ...
        if is_epoch:
            chk_fname_link = (
                self.chk_prefix + str(self.chk_global_id) + "_" + str(epoch)
            )
        ...
        if synchronous:
            ...
            if not is_epoch:
                ...
                filepath_link_sync = self._get_full_path(
                    chk_fname_link_sync, epoch=True
                )
                self.chk._serialize_and_persist_direct(
                    self.chk.latest_snapshot,
                    filepath_sync,
        ...
        success = self.chk._snapshot(
            self.active_snapshot.value, additional_state=additional_snapshot
        )
        ...
        if use_thread:
            ...
            # fn = self.mp_context.Process
            fn = getattr(threading, "Thread")
        print("Spawn save Function is {}".format(fn))
        ...
        if not is_epoch:
            ...
            self.chk_process = fn(
                target=self.chk._serialize_and_persist,
                args=[
                    filepath,
                    self.chk.latest_snapshot,
    ...
    def save_cpu(
        ...
        if synchronous:
            self.chk._snapshot_and_persist_async(
                filepath,
                self.active_snapshot,
        ...
        if use_thread:
            ...
            # fn = self.mp_context.Process
            fn = getattr(threading, "Thread")
        print("Spawn Function is {}".format(fn))
        if not is_epoch:
            keywords = {
                "iter_chk": self.available_chk_iters,
                "overwrite": self.overwrite,
            ...
            self.chk_process = fn(
                target=self.chk._snapshot_and_persist_async,
                args=[
                    filepath,
                    self.active_snapshot,
                    self.in_progress_snapshot,
    ...
    def initalize_chk_dir(self):
        if os.path.exists(self.chk_dir):
            ...
            chk_files = [
                ...
                for f in os.listdir(self.chk_dir)
                if isfile(os.path.join(self.chk_dir, f))
            ]
            ...
            if os.path.exists(epoch_chk_dir):
                epoch_chk_files = [
                    os.path.split(f)[0]
                    for f in os.listdir(epoch_chk_dir)
                    if isfile(os.path.join(epoch_chk_dir, f))
                ]
